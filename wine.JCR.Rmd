...
wine_recode <- wine_f %>%
  mutate(grade = ifelse(points>=90,1,0)) %>%
  mutate(country_level = recode(country,
                                "US" = "US", "France" = "France", "Italy" = "Italy", .default = "Other")) %>%
  mutate(province_level = recode(province,
                                "California" = "California", "Washington" = "Washington", "Bordeaux" = "Bordeaux",
                                "Tuscany" = "Tuscany", .default = "Other")) %>%
  mutate(variety_level = recode(variety,
                                "Pinot Noir" = "Pinot Noir", "Chardonnay" = "Chardonnay", "Cabernet Sauvignon" = "Cabernet Sauvignon",
                                "Red Blend" = "Red Blend", "Bordeaux-style Red Blend" = "Bordeaux-style Red Blend", .default = "Other")) %>%
...
```{r}
# modelling using GLM
model <- glm(grade ~ ., family = binomial, data=wine_recode)
summary(model)
```
